...
test('release data is the shared product and SEO release source', async () => {
  ...
  assert.equal(releases['leke-picker'].version, '1.1.0');
  assert.equal(releases.guigelei.version, '1.5.0');
  ...
  assert.equal(routes.find((route) => route.path === '/products/leke-picker').software.version, '1.1.0');
  assert.equal(routes.find((route) => route.path === '/products/guigelei').software.version, '1.5.0');
});
