...
const payload = { product:'leke-picker',version:'1.1.0',system:'windows-11',issueType:'install',description:'安装之后无法启动应用，请协助排查具体原因。',contact:'teacher@example.com',name:'王老师',privacyConfirmed:true,website:'',turnstileToken:'ok' };
...
test('only reports success after mail acceptance', async () => {
  const env={ALLOWED_SUPPORT_ORIGINS:'https://lekeopen.com',TURNSTILE_SECRET_KEY:'secret',RESEND_API_KEY:'secret',SUPPORT_MAIL_FROM:'support@lekeopen.com',SUPPORT_MAIL_TO:'support@lekeopen.com',SUPPORT_RATE_LIMIT:{get:async()=>null,put:async()=>{}}};
  ...
  const response=await onRequestPost({request,env,waitUntil(){},data:{},next(){},params:{}},{fetchImpl:async(url)=>String(url).includes('turnstile')?new Response('{"success":true}'):new Response('{"id":"email-id"}'),randomBytes:new Uint8Array([1,2,3,4]),now:new Date('2026-08-13')});
