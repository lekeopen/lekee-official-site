...
test('support page exposes accessible ordinary-user form',()=>{assert.match(app,/path="support"/);assert.match(page,/<h1[^>]*>产品帮助与反馈/);for(const text of ['无需 GitHub 账号','contact@lekeopen.com','privacyConfirmed','turnstileToken','render=explicit','turnstile.render'])assert.match(page,new RegExp(text));});
