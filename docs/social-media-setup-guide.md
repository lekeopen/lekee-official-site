...
官网构建时会生成 `publish-queue.json`，其中 `channels` 已包含 `x`。实际发帖仍建议通过 Make.com 监听 RSS 或发布队列后调用 X 模块完成。

### 推荐流程
1. 在 Make.com Scenario 中添加 RSS 触发器，来源使用 `https://lekeopen.com/rss.xml`。
2. 添加 X/Twitter 模块，动作选择发布文本帖。
3. 连接乐可开源的 X 账号：`https://x.com/lekeopen`。
4. 贴文内容建议控制在 X 字数限制内：
```
{{1.title}}
...
阅读原文：{{1.link}}
```
5. 开启 Scenario，并用 Make.com 的 `Run once` 做一次抓取与发布验证。
...
如果后续改为读取 `publish-queue.json` 而不是 RSS，请让自动化只处理 `channels` 包含 `x` 且 `status` 为 `pending` 的条目，避免重复发布。
...
- **X**：如果 Make.com 的 X 模块连接失败，通常需要重新授权账号，或检查 X 开发者/API 权限是否仍可用。
