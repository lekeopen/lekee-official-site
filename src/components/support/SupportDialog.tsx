...
export default function SupportDialog({ productId }: { productId: 'leke-picker' | 'guigelei' }) {
  const productName = productId === 'leke-picker' ? '乐可点名' : '归个类';
  ...
  return <>
    <button ref={trigger} type="button" onClick={() => setOpen(true)} className="inline-flex min-h-11 items-center font-semibold text-blue-700 hover:text-blue-900">问题反馈与使用帮助</button>
    {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/60 p-0 sm:p-6" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <div ref={dialog} role="dialog" aria-modal="true" aria-label={`${productName}帮助与反馈`} tabIndex={-1} className="relative h-full w-full overflow-y-auto bg-gray-50 outline-none sm:h-auto sm:max-h-[90vh] sm:max-w-3xl sm:rounded-2xl">
        ...
        <div className="px-4 pb-10 text-center sm:px-6 lg:px-8">
          <a href={`/support/?product=${productId}`} className="inline-flex min-h-11 items-center font-semibold text-gray-600 hover:text-gray-900">打开完整反馈页</a>
        </div>
      </div>
    </div>}
  </>;
