...
## Unreleased - 产品承接工程（待人工确认）

- feat: 在“产品与项目”中增加乐可产品分区与两款产品入口。
- feat: 新增乐可点名 v1.1 与归个类 v1.5 独立产品页、真实界面、下载信息、版本要求、隐私说明和 FAQ。
- content: 新增乐可点名 v1.1.0 与归个类 v1.5.0 两篇产品动态。
- feat: 增加 GitHub Release 安装包白名单下载统计与失败降级。
- SEO: 为产品页增加独立 OG、关键词映射、SoftwareApplication 和三级面包屑结构化数据。
- privacy: 使用 Clarity 白名单产品事件，明确不传递名单或文件数据。
- docs: 记录开源/私有发布边界、人工确认门禁、分仓计划和回滚方案。
- release: 本条仅记录未发布工程变更；未提交、未推送、未合并、未部署。
